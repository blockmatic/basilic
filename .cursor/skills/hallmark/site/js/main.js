...
const StorageKey = 'hallmark-theme'
...
const HeroTitle = 'A design skill that refuses to look AI-generated.'
...
;(() => {
  ...
  const CacheKey = 'hallmark-star-count:' + Repo // key by repo — renames auto-invalidate
  ...
  fetch(`https://api.github.com/repos/${Repo}`, {
    ...
    .then(d => {
      ...
      try {
        localStorage.setItem(CacheKey, JSON.stringify({ n, t: Date.now() }))
      } catch (e) {}
    })
    .catch(() => {
...
function setPressed(theme) {
  ...
  const genre = ThemeGenres[theme] || 'editorial'
...
const TTooltipKey = 'hallmark-t-tooltip-seen'
const TTooltipDelayMs = 5000
const TTooltipAutoHideMs = 8000
const TTooltipFadeMs = 240
...
function tTooltipSeen() {
  try {
    return localStorage.getItem(TTooltipKey) === '1'
  } catch (e) {
    return false
  }
}
...
function markTTooltipSeen() {
  try {
    localStorage.setItem(TTooltipKey, '1')
  } catch (e) {}
}
...
function showTTooltip() {
  ...
  tTooltipAutoHideTimer = setTimeout(hideTTooltip, TTooltipAutoHideMs)
}
...
if (tTooltipEl && !tTooltipSeen()) {
  tTooltipTimer = setTimeout(showTTooltip, TTooltipDelayMs)
...
const EasterWindowMs = 3200
const EasterThreshold = 12 // ≈ 3.8 presses/sec
const EasterVisibleMs = 3400 // total time the overlay stays up
const EasterFadeMs = 360 // matches the fade-out animation
const EasterCooldownMs = 15000
const EasterPunchlines = [
  'theme connoisseur.',
  'easy on the keys.',
  'speed-run noted.',
  'calm down, designer.',
...
function showEasterEgg() {
  ...
  if (lineEl)
    lineEl.textContent = EasterPunchlines[Math.floor(Math.random() * EasterPunchlines.length)]
  ...
  easterDismissTimer = setTimeout(hideEasterEgg, EasterVisibleMs)
}
...
document.addEventListener('keydown', e => {
  ...
  if (e.key === 't' || e.key === 'T') {
    ...
    while (tStamps.length && now - tStamps[0] > EasterWindowMs) tStamps.shift()
    tStamps.push(now)
    if (tStamps.length >= EasterThreshold && now - easterLastFired > EasterCooldownMs) {
      easterLastFired = now
      tStamps.length = 0
      showEasterEgg()
