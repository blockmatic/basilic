...
Type `/` in chat for Matt’s pack (`/grill-me`, `/implement`, `/tdd`) and Basilic `/w-*` (`/w-plan`, `/w-council`, `/w-ship`). `/w-plan` explores via `/w-council` first. Tech skills load when relevant, or `@.agents/skills/<name>`.
