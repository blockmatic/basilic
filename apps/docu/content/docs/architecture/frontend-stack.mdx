...
- **`useLinkWallet`** (`@/hooks/use-link-wallet`): link connected wallet to account (Bearer required, verified email required)
...
**Post-login home:** After auth (magic link or wallet), users are redirected to `/`. The home page renders merged dashboard content including Link wallet.
