...
description: "ESM-first module architecture with TypeScript 7, dual-mode exports, subpath exports, ts-reset, and the native Go compiler for fast builds and typechecks."
---
...
- **Fast compilation/typecheck**: TypeScript 7 (`tsc`) is the native Go compiler; we use it for builds and `--noEmit` typechecks
...
Node.js apps use native ESM and typically run TypeScript in development via `tsx` (Node ESM loader), while builds/typechecks use TypeScript 7 (`tsc`).
...
**Build / watch / typecheck (TypeScript 7):**
...
TypeScript 7 is the native Go port of the compiler. Install it as `"@typescript/native": "npm:typescript@^7.0.2"` and run `tsc` — the same command as prior versions, now 8–12× faster. TypeScript 7.0 does not ship a programmatic API (planned for 7.1), so tools that import `typescript` (typescript-eslint, Next.js webpack path resolution) keep TypeScript 6 via `"typescript": "npm:@typescript/typescript6@^6.0.2"`. Next.js 16.3 defaults to the TypeScript CLI (`typescript/bin/tsc`), which that alias does not provide — set `experimental.useTypeScriptCli: false` so Next uses the TypeScript 6 API instead.
...
- ✅ Use TypeScript 7 `tsc` for builds and `--noEmit` typechecks (default across apps/packages)
...
The monorepo uses **TypeScript 7** (`tsc`, native Go compiler) with strict mode enabled and enhanced type safety via `@total-typescript/ts-reset`. ESLint keeps TypeScript 6 via `@typescript/typescript6` until 7.1 ships a stable API.
...
- ES2022 target
- ESNext modules
- `types: ["*"]` (TypeScript 7 defaults `types` to `[]`; this restores automatic `@types/*` inclusion)
...
TypeScript conventions are defined in `.cursor/rules/base/typescript.mdc` and enforced via ESLint and Biome. Key rules: strict mode, no `any`, ESM (`fileURLToPath(import.meta.url)` instead of `__dirname`), named exports, RORO pattern, thin wrappers avoided, lodash per-function imports.

**Exclusions:**
- `packages/core/src/gen/` — Autogenerated OpenAPI client code; not hand-maintained
- `.cursor/skills/**/templates/` — Example/snippet code for skills

## Related Documentation

- [Package Conventions](/docs/development/package-conventions) - Package organization and naming
