...
**Pattern B** (`@repo/utils`, `@repo/error`, `@repo/email`, `@repo/db`): `source` for bundlers → `src/` (except `@repo/db`, which omits `source` because Next must not import it); `node` / `import` / `default` for compiled `dist/` so serverless resolvers do not hit `ERR_MODULE_NOT_FOUND`. `@repo/db` keeps real schema `.ts` and `.sql` files (tsc emit, not a tsup schema bundle) so drizzle-kit can generate migrations.
