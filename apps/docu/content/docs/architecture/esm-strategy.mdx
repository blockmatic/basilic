...
**Pattern B** – `source` condition for bundlers, `import` for Node (packages like `@repo/utils`, `@repo/sentry`):
...
Next.js apps resolve the `source` condition first (via webpack config) to consume TypeScript directly.
