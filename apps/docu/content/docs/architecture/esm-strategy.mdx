...
**Pattern B** (`@repo/utils`, `@repo/error`, `@repo/email`, `@repo/db`): `source` for bundlers → `src/` (except `@repo/db`, which omits `source` because Next must not import it); `node` / `import` / `default` for compiled `dist/` so serverless resolvers do not hit `ERR_MODULE_NOT_FOUND`. `@repo/db` keeps real schema `.ts` and `.sql` files (tsc emit, not a tsup schema bundle) so drizzle-kit can generate migrations. Pattern B `tsup` leaves sibling `@repo/*` packages external. `noExternal` follows `import` → `dist/` and races `pnpm dev` watchers that `clean` that folder. A package-name `external` entry does not match subpaths (`@repo/utils/logger/server`); use `/^@repo\/utils/`.
...
Node dev: `tsx`. Next: `transpilePackages` plus webpack `conditionNames: ['source', …]` and `.js` → `.ts` `extensionAlias` (required in Next 16.3.5 — Turbopack lacks `resolveExtensionAlias`). Scripts pass `--webpack` until Turbopack supports workspace `.js` imports. Import `@repo/*`, not relative paths into `packages/`.
