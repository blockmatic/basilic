...
flowchart TB
  subgraph Packages["packages/"]
    ...
    Markets["markets — CoinGecko + Binance"]
  end
  ...
  subgraph Apps["apps/"]
    ...
    Agents["agents — eve command and chat (sibling, not scored)"]
...
**Packages:** `core` (generated, no app runtime), `react` (handwritten hooks on core), `ui`, `email`, `error`, `utils`, `cli`, `db` (Drizzle schema + named fns; Fastify + eve host), `markets` (CoinGecko + Binance; Fastify + eve host).
