...
- Rate limit: in-memory per client IP (`RATE_LIMIT_MAX` / `RATE_LIMIT_TIME_WINDOW`). 429 responses include IETF `RateLimit` / `RateLimit-Policy` / `Retry-After` and legacy `x-ratelimit-*`. Behind a reverse proxy, set `TRUST_PROXY=true` so Fastify honors `X-Forwarded-For` (Fastify 5 treats numeric hop counts as fail-closed). Local-only, not shared across instances.
