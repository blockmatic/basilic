...
Layers: local pre-commit, `security.yml` on every PR and push to main, DeepSec on trusted same-repo PRs (`deepsec.yml`), and Fastify request hardening. Install scanners with `pnpm run setup` (gitleaks **8.30.1** with pinned SHA-256, osv-scanner **2.6.0** from GitHub release tags). Manual install: [gitleaks](https://github.com/gitleaks/gitleaks#installation), [OSV Scanner](https://google.github.io/osv-scanner/installation/), [TruffleHog](https://github.com/trufflesecurity/trufflehog#installation).
...
Lockfile integrity (`--frozen-lockfile`), gitleaks, TruffleHog (filesystem + history), OSV on `pnpm-lock.yaml`, `pnpm audit` (`--ignore-registry-errors` so a registry timeout is not a finding). DeepSec is a separate workflow. See [GitHub Actions](/docs/deployment/github-actions).
...
- CORS: `ALLOWED_ORIGINS` must be explicit origins in production. Omit the var or set `*` and production uses the `WEB_APP_URL` origin so the Function still boots (discovery GETs). Fastify [`plugins/cors.ts`](https://github.com/blockmatic/basilic/blob/main/apps/api/src/plugins/cors.ts) is the source of truth. Do not set `Access-Control-Allow-Origin` in `vercel.json` or other deployment headers. Dev/test may use `*`.
- Rate limit: in-memory per client IP (`RATE_LIMIT_MAX` / `RATE_LIMIT_TIME_WINDOW`). `GET /coins`, `POST /coins/query`, `/coins/watches`, and `GET /coins/:assetId/candles` use a tighter cap (`COINS_RATE_LIMIT_MAX`, default 10) than `/ai/*` (`AI_RATE_LIMIT_MAX`, default 20). 429 responses include IETF `RateLimit` / `RateLimit-Policy` / `Retry-After` and legacy `x-ratelimit-*`. Behind a reverse proxy, set `TRUST_PROXY=true` so Fastify honors `X-Forwarded-For` (Fastify 5 treats numeric hop counts as fail-closed). Local-only, not shared across instances.
...
- Outbound market HTTP allowlists only `api.coingecko.com` and `data-api.binance.vision` (`packages/markets` policy). Wallet HTTP allowlists `api.g.alchemy.com` (`packages/onchain` policy). Redirects off those lists are not followed. Cross-origin redirects are rejected even when the target host is allowlisted, so request headers are not replayed to another origin. Binance `ticker/24hr` always includes `symbol`.
- JWT access and refresh tokens are signed and verified with **HS256** only (`@fastify/jwt`).
- Eve Chat ingress does not fetch client-supplied file URLs; only `data:` file parts are accepted. Remote URLs are rejected in `apps/agents/lib/ingress.ts` before the session is created.
...
Secrets in env (not a threat model): `ENCRYPTION_KEY`, `JWT_SECRET`, OAuth client secrets, `RESEND_API_KEY`, AI keys, `SENTRY_DSN` (unused until Sentry is re-enabled), `POSTGRES_URL`, `AI_GATEWAY_API_KEY`, `EXPO_TOKEN`.
...
- Web3 nonce, verify, and exchange under `/auth/web3/*`
...
Named threat model; data classification list; in-product AI tool permission matrix; a dated accepted-risk register with owner and next review; refresh ownership / concurrency protocol (see [Authentication](/docs/architecture/authentication)).
