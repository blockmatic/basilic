...
**CI:** `.github/workflows/deepsec.yml` runs on same-repo pull requests only (forks skip; no secrets). The `analyze` job installs `.deepsec` only, runs Sol GPT `process --diff` against the PR base, and fails on net-new findings. A second job posts the comment; it never executes PR code. Add the `AI_GATEWAY_API_KEY` repository secret (Vercel AI Gateway key, `vck_…`) in **Settings → Secrets and variables → Actions**. `OPENAI_API_KEY` works as a fallback for Codex direct auth. Without either secret, the workflow fails before processing.

The `secret-env-var` matcher is excluded so DeepSec does not duplicate Gitleaks/TruffleHog.
