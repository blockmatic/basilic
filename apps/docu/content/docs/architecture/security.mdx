...
- CORS: `ALLOWED_ORIGINS` must be explicit origins in production (boot fails on `*` or empty). Dev/test may use `*`. Config: `apps/api/src/plugins/cors.ts`.
- Rate limit: in-memory per client IP (`RATE_LIMIT_MAX` / `RATE_LIMIT_TIME_WINDOW`, `trustProxy`); local-only, not shared across instances.
