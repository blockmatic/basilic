...
2. **CI workflows** - Secrets and dependency CVEs on every pull request and push to main (`security.yml`)
3. **Source-code review** - DeepSec reviews the PR diff on same-repo pull requests (`deepsec.yml`)
...
Install all required security tools automatically with `pnpm run setup` (see [Getting Started](/docs/development)). For manual installation, see the instructions below.
...
# DeepSec: regex scan (no AI)
pnpm security:deepsec:scan

# DeepSec: AI review of the diff vs origin/main
pnpm security:deepsec:process:diff

# DeepSec: full-repo AI review (local only; needs model auth)
...
DeepSec is **not** in this workflow. Same-repo PRs run `deepsec.yml` separately (see below).

## Source-code review (DeepSec)

[DeepSec](https://deepsec.sh) is agent-powered SAST. It does not replace Gitleaks, TruffleHog, OSV Scanner, or `pnpm audit`. Those tools own secrets and lockfile CVEs. DeepSec reviews application code.

The workspace lives in `.deepsec/` at the repo root. It is not a pnpm workspace package. Commit `deepsec.config.ts`, `INFO.md`, and generated matchers. Scan output (`data/*/files/`, `data/*/runs/`), `.env.local`, and `node_modules` stay gitignored.

DeepSec is not in pre-commit and not in `security.yml`.

**Agents:** CI and default local `process` use GPT-5.6 Sol on the Codex harness (`--agent codex --model gpt-5.6-sol --thinking-level medium`). Cursor Grok 4.6 is the alternate: `pnpm security:deepsec:process:diff:grok` (`--agent pi --model xai/grok-4.6`). Both route through Vercel AI Gateway. Claude is not used.

**Local:** `pnpm security:deepsec:scan` is regex-only and free. `process` / `process:diff` need `AI_GATEWAY_API_KEY`. A full-repo `process` is an operator follow-up, not required for every commit.

**CI:** `.github/workflows/deepsec.yml` runs on same-repo pull requests from OWNER, MEMBER, or COLLABORATOR (forks and untrusted authors skip; no secrets). `analyze` gets `AI_GATEWAY_API_KEY` only after that trusted-author gate; GitHub also requires maintainer approval for first-time contributor workflow runs. The job checks out the PR for scan input, overlays `.deepsec` from the PR base SHA when present, runs Codex `process --files-from` on `git diff --name-only BASE_SHA...HEAD`, and fails on net-new findings. Codex uses GPT-5.6 Sol via Vercel AI Gateway (`https://ai-gateway.vercel.sh/v1`). A second job posts the comment; it never executes PR code and never receives the key. Add `AI_GATEWAY_API_KEY` (`vck_…`) in **Settings → Secrets and variables → Actions**. Without that secret, `analyze` is skipped.
...
- **`.github/workflows/security.yml`** - CI workflow for secret and dependency checks
- **`.github/workflows/deepsec.yml`** - PR diff source-code review (same-repo only)
- **`.deepsec/`** - DeepSec workspace (`deepsec.config.ts`, `INFO.md`)
...
### CI failing on DeepSec install (`minimumReleaseAge`)

`deepsec.yml` runs `pnpm install --frozen-lockfile` in `.deepsec`. pnpm 11 rejects lockfile entries younger than 24 hours. If the version is a known security patch, add the exact version to `minimumReleaseAgeExclude` in `.deepsec/pnpm-workspace.yaml` (for example `qs@6.16.0`). Do not pin back to a vulnerable release and do not set `minimumReleaseAge` to `0`.

### CI failing on DeepSec credentials

`deepsec process` needs model auth in CI (no local Codex login). Add `AI_GATEWAY_API_KEY` (`vck_…` from [Vercel AI Gateway](https://vercel.com/docs/ai-gateway)) to repository secrets. `analyze` receives it only after the trusted-author gate (OWNER, MEMBER, or COLLABORATOR on a same-repo PR). Without the secret, or for forks and untrusted authors, `analyze` is skipped. The workflow writes `.deepsec/.env.local` from the secret before running. The comment job never receives the key.
...
Cross-Origin Resource Sharing (CORS) allows all origins. Security is enforced via JWT authentication rather than origin restriction.

- **Allowed Origins**: All (`*`)
- **Allowed Methods**: GET, POST, PUT, DELETE, PATCH, OPTIONS
- **Allowed Headers**: Content-Type, Authorization, X-Requested-With
- **Credentials**: Disabled (JWT-only auth, no cookies required)
...
Rate limiting is configured in `apps/api/src/plugins/rate-limit.ts`.
...
Security logging is implemented in `apps/api/src/plugins/security.ts` and `apps/api/src/lib/security.ts`.
...
API keys are stored hashed (SHA-256) and never in plaintext. Use timing-safe comparison for validation. Rotate keys via revoke + create when compromised or for routine rotation.

### API Security Best Practices

1. **Always use HTTPS** - Enforced by HSTS header in production
2. **Configure rate limits** - Adjust based on your API usage patterns
3. **Monitor security logs** - Review security events regularly
4. **Keep dependencies updated** - Run `pnpm security:audit` and `pnpm security:osv` regularly
5. **Validate all inputs** - Use Zod schemas for all request validation
6. **Sanitize error messages** - Don't expose internal details in production
...
1. **Review security logs** - Check `apps/api/src/plugins/security.ts` logs
...
- [DeepSec documentation](https://deepsec.sh)
...
- [API Development](/docs/architecture/api) - Fastify security headers and CORS configuration
