...
**CI:** `.github/workflows/deepsec.yml` runs on same-repo pull requests from OWNER, MEMBER, or COLLABORATOR (forks and untrusted authors skip; no secrets). The `analyze` job checks out the PR for scan input, overlays `.deepsec` from the PR base SHA when present, runs Codex `process --files-from` on `git diff --name-only BASE_SHA...HEAD`, and fails on net-new findings. GPT-5.6 Sol is used when `AI_GATEWAY_API_KEY` is set (Codex is pointed at `https://ai-gateway.vercel.sh/v1`). `OPENAI_API_KEY` alone falls back to `gpt-5.5` on OpenAI direct auth — `gpt-5.6-sol` is a Gateway model and is not sent to `api.openai.com`. A second job posts the comment; it never executes PR code. Add `AI_GATEWAY_API_KEY` (recommended, `vck_…`) or `OPENAI_API_KEY` in **Settings → Secrets and variables → Actions**. Without either secret, the workflow fails before processing.
...
`deepsec process` needs model auth in CI (no local Codex login). Add `AI_GATEWAY_API_KEY` (`vck_…` from [Vercel AI Gateway](https://vercel.com/docs/ai-gateway)) to repository secrets, or `OPENAI_API_KEY` as a fallback. The workflow writes `.deepsec/.env.local` from the secret before running.
