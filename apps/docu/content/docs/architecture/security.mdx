...
- Outbound market HTTP allowlists only `api.coingecko.com` and `data-api.binance.vision` (`packages/markets` policy). Redirects off that list are not followed. Cross-origin redirects are rejected even when the target host is allowlisted, so request headers are not replayed to another origin. Binance `ticker/24hr` always includes `symbol`.
