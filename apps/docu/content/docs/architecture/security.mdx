...
`deepsec process` needs model auth in CI (no local Codex login). Add `AI_GATEWAY_API_KEY` (`vck_…` from [Vercel AI Gateway](https://vercel.com/docs/ai-gateway)) to repository secrets. Without it, `analyze` is skipped. The workflow writes `.deepsec/.env.local` from the secret before running.
