...
- Outbound market HTTP allowlists only `api.coingecko.com` and `data-api.binance.vision` (`apps/api/src/lib/markets/policy.ts`). Redirects off that list are not followed. Binance `ticker/24hr` always includes `symbol`.
