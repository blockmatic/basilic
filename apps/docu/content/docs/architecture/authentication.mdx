...
- **BFF for browsers**: Next.js stores tokens in HttpOnly cookies; only callbacks (OAuth, magic link verify, Web3 token exchange) go through the BFF—login requests go directly to the API
...
2. User authorizes on GitHub → GitHub redirects to `OAUTH_GITHUB_CALLBACK_URL` (Next.js BFF) with `?code=&state=`
3. BFF calls `POST /auth/oauth/github/exchange` with `{ code, state }` → Fastify exchanges code, creates/upserts user + account, returns `{ token, refreshToken }`
4. BFF sets HttpOnly cookies and redirects to `/`

Login requests go directly to the API; only the callback (token exchange + cookie setting) goes through the Next.js BFF.

**Setup:** Create a [GitHub OAuth App](https://github.com/settings/applications/new). Set callback URL to `http://localhost:3000/api/auth/oauth/github/callback` (dev) or your production URL. Add `GITHUB_CLIENT_ID`, `GITHUB_CLIENT_SECRET`, and `OAUTH_GITHUB_CALLBACK_URL` to Fastify env.
...
- **Proxying**: Selected paths (`magic-link` → `magiclink`, `session/user`, etc.) are proxied to Fastify. OAuth authorize is not proxied—clients use `useOAuthLogin` to call `/auth/oauth/github/authorize-url` directly.
