...
EIP-155 and Solana verify routes store short-lived callback JWTs in `web3_callback` **encrypted at rest** (AES-GCM, same helpers as passkey callbacks). `POST /auth/web3/exchange` decrypts before returning tokens to the client. Decrypt failure returns 401 `INVALID_OR_EXPIRED_CODE`. `POST /auth/passkey/exchange` uses the same encrypt helpers on `passkey_callback` and returns 401 `INVALID_OR_EXPIRED_CODE` when decrypt fails. Passkey **verify** consumes the auth challenge before WebAuthn verification (one-shot; concurrent reuse returns `EXPIRED_CHALLENGE`). Credential counters use compare-and-swap on update. Exchange requires a non-empty stored `callbackOrigin` and matching request `Origin` / `X-Callback-Origin`. See [Login route rate limit](/docs/architecture/security#login-route-rate-limit-as-shipped) for which auth routes share the tight Fastify cap.
