...
**Framework support:** Wallet integration lives in `apps/next` (`@/hooks/`, `@/wallet/`). `@repo/react` exposes only verify hooks: `useVerifyWeb3Auth`, `useVerifyLinkWallet`. App-local hooks: `useWallet`, `useWalletAuth`, `useLinkWallet`, `useWeb3Nonce`. `useLinkEmail` remains in `@repo/react`. Vanilla (fetch) and Vue (composable) also supported.
