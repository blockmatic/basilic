...
- **Usage**: `Authorization: Bearer bask_<prefix>_<secret>` — same header as JWT; Fastify distinguishes by prefix
- **Endpoints** (JWT Bearer required): `POST /account/apikeys` (create), `GET /account/apikeys` (list), `DELETE /account/apikeys/:id` (revoke)
...
```mermaid
flowchart LR
  subgraph LinkWalletFlow [Link wallet]
