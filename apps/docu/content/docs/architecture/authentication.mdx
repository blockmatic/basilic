...
- **No Fastify proxy**: Clients call Fastify directly (`NEXT_PUBLIC_API_URL`). Next.js API exists only for cookie updates (refresh, update-tokens).
- **Minimize Next.js API**: Prefer auth pages (callbacks, logout) over API routes.

## Overview

Authentication is implemented in `apps/fastify` as a **JWT Bearer** system backed by **Drizzle + PostgreSQL/PGLite** session state. All auth methods require a **verified email**: magic link and OAuth set `emailVerified` on sign-in; Web3 is link-only (users must sign up via email or OAuth first, then link a wallet).
...
**Error codes:** `INVALID_NONCE`, `EXPIRED_NONCE`, `INVALID_SIGNATURE` for consistent UI handling. Wallet rejection (e.g. user denies signing) is surfaced as a non-fatal error. Web3 verify: `SIGNUP_REQUIRED` (wallet not linked—sign up with email or OAuth first), `EMAIL_VERIFICATION_REQUIRED` (linked user not verified). Account link: `WALLET_ALREADY_LINKED`, `EMAIL_ALREADY_IN_USE`, `EXPIRED_TOKEN`.

**Web3 link-only:** Users must sign up via email (magic link) or OAuth first. Web3 verify only logs in when the wallet is already linked to a verified user.

**JWT cookie refresh:** After link wallet or profile edit, the client receives `{ token, refreshToken }` from the verify endpoint. Call `POST /api/auth/update-tokens` with those values to update cookies. In `apps/next`, use `updateAuthTokens` from `@/lib/auth-client`.
...
sequenceDiagram
  ...
  FastifyAPI->>FastifyAPI: verify nonce + signature, require linked verified user
...
- **Account linking** (Bearer required — JWT or API key; verified email required)
...
**Account link error codes**: `WALLET_ALREADY_LINKED`, `EMAIL_ALREADY_IN_USE`, `EMAIL_VERIFICATION_REQUIRED`
...
- `POST /api/auth/refresh`: reads refresh token from cookie, calls Fastify refresh, sets new cookies. Used by core client on 401.
- `POST /api/auth/update-tokens`: accepts `{ token, refreshToken }`, sets cookies. Used after link wallet or profile edit.
...
- **`verification`**: single-use tokens (stored hashed, with TTL); `type` = `magic_link` | `oauth_state`
