...
- **Resolve user**: `POST /auth/passkey/resolve-user` with `{ userHandle }` (base64url from assertion) → returns `{ email }` for "Login as {email}" display. Rate limited (10/min per IP).
