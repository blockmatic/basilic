...
- Coin board: RSC `createLoader` reads the URL `SearchQuery` and fetches `GET /coins` via the BFF client so first HTML matches the query. The client island hydrates the same parsers with shallow `history: 'push'` and refetches with `@repo/core` JWT. Do not set `shallow: false` on those writes.
...
- **RSC:** home coin board (`createLoader` + `listCoins` + `composeSurface`) and profile (`getUserInfo` in the page, `initialData` on `useUser`).
- **Client TanStack:** coin-board island hydrates a json-render `Renderer` with `StateProvider` `{ coins, sync }`, then refetches and watch stars; settings/security (API keys, passkeys, TOTP). Security surfaces are mutation-first and need WebAuthn or OTP browser APIs. Do not duplicate their GETs in Server Components; an empty first paint until hydration is accepted.
