...
- **RSC:** home coin board (`createLoader` + `listCoins`) and profile (`getUserInfo` in the page, `initialData` on `useUser`).
- **Client TanStack:** coin-board island refetches and watch stars; settings/security (API keys, passkeys, TOTP). Security surfaces are mutation-first and need WebAuthn or OTP browser APIs. Do not duplicate their GETs in Server Components; an empty first paint until hydration is accepted.
...
Frontend apps use Playwright E2E only. See [E2E Testing](/docs/testing/e2e-testing). Playwright specs are not a substitute for the `/ui` rendered pass (desktop and mobile screenshots plus keyboard). Visual-regression CI is not shipped.
