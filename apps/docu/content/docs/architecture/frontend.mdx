...
- Unmatched paths render `apps/web/app/not-found.tsx` (HTTP 404). When dynamic resource routes exist, call `notFound()` for missing resources — not inline “not found” JSX with a 200 status.
...
- **Errors:** `app/error.tsx` and the shared `ErrorBoundary` report via `captureError` from `@repo/error/nextjs`, same as `global-error.tsx`.
