...
`@repo/ui` owns shadcn-style components on Base UI primitives, and Tailwind 4. Import `@repo/ui/components/*`. Primitive composition: `@repo/ui/base`. Prototype in v0 if useful, then install into `@repo/ui` (`components.json` style `base-vega`) and consume from apps.
