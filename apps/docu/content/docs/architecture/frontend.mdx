...
Frontend apps (for example `apps/web` and `apps/docu`) are built on **Next.js** and share UI, data access, and utilities through monorepo packages.
...
flowchart TB
  subgraph Apps["Applications (Next.js)"]
    Web["apps/web<br/>(product web app)"]
...
Wallet auth, link, sign, and send: use `@repo/react` hooks via `WalletProvider`; apps supply wagmi + Solana adapters. See [Frontend Stack](/docs/architecture/frontend-stack) for hooks, `WalletAdaptersInjector`, and mermaid.
...
Frontend apps are validated via **Playwright E2E tests** instead of separate unit or integration suites. See [Frontend Testing](/docs/testing/frontend-testing) for the E2E-only frontend testing strategy.
