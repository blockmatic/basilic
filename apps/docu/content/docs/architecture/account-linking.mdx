...
|------------|--------------|------------|
| Effect | Replaces `users.email` | Sets `users.email` when it is currently null |
| Flow | Request → verify code/link → update | Request → verify link → set email |
| Use case | User changes email address | Authenticated session with `users.email` null requests a primary email |
...
- Uses `oauth_link_state` (distinct from `oauth_state` for sign-in). Both are `verification.type` values, not tables — see [Data](/docs/architecture/data)
...
`POST /account/link/wallet/verify` binds `domain` the same way login verify does (`ALLOWED_ORIGINS` hostnames) and requires `users.email`. Accounts without email get **400** `EMAIL_REQUIRED`. Settings lists `linkedWallets` from `GET /auth/session/user`, opens the same custom modal, and unlinks with `DELETE /account/link/wallet/:id`. New addresses cannot self-register via SIWE/SIWS; that path returns `WALLET_NOT_LINKED`. `POST /account/link/email/request` remains available to any authenticated session whose `users.email` is null.
