...
|------------|--------------|------------|
| Effect | Replaces `users.email` | Sets `users.email` when it is currently null (legacy Web3-only account) |
...
`POST /account/link/wallet/verify` binds `domain` the same way login verify does (`ALLOWED_ORIGINS` hostnames) and requires `users.email`. Accounts without email get **400** `EMAIL_REQUIRED`. Settings lists `linkedWallets` from `GET /auth/session/user`, opens the same custom modal, and unlinks with `DELETE /account/link/wallet/:id`. New addresses cannot self-register via SIWE/SIWS; that path returns `WALLET_NOT_LINKED`. Link-email remains for **legacy** wallet-only rows created before this lock.
