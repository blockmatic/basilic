...
- **Clear migration path**: Adopter may point `DATABASE_URL` at another Postgres host. Basilic ships Vercel + Postgres; GCP/AWS are not a Basilic destination.
...
- **Strategic approach**: Basilic ships Vercel + Postgres (`DATABASE_URL`). Other hosts (Supabase, GCP, AWS, on-prem) remain an adopter choice via the connection string, not a Basilic destination.
