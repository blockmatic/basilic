...
flowchart TD
    ...
    UsePostgres --> MigratePhase[pnpm db:migrate]
    MigratePhase --> ConnectPG[Connect to PostgreSQL]
    ConnectPG --> AppReady
...
- **Local dev (PostgreSQL)**: Migrations run via **`pnpm db:migrate`**, not when you run `pnpm build` / `pnpm --filter @repo/api build` alone. To wipe local Supabase and re-apply schema + seed, use **`pnpm reset`** (repo root) or **`pnpm reset`** in `apps/api`. **PGLite**: SQL assets must be present next to compiled `migrate.js`; empty list is a hard error when `PGLITE=true` or `NODE_ENV=test`
...
Run `pnpm --filter @repo/api db:stop` before switching to another project's Supabase instance (e.g. different repo); only one Supabase runs per host.
