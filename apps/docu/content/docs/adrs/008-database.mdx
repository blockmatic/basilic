...
- **Migration path**: Easy - only requires changing `POSTGRES_URL`
...
- **Migration path**: Change `POSTGRES_URL`
...
| **Migration path** | ✅ Change POSTGRES_URL | ✅ Change POSTGRES_URL | ✅ Standard | ❌ Rewrite needed | ❌ Rewrite needed |
...
- **Portability**: Migration requires only changing `POSTGRES_URL` - no code changes
- **Clear migration path**: Adopter may point `POSTGRES_URL` at another Postgres host. Basilic ships Vercel + Postgres; GCP/AWS are not a Basilic destination.
- **Complements Fastify**: Both stay portable; Basilic’s shipped path is Vercel.
...
- Migrations run at **build time** via `pnpm db:migrate` (Vercel: after `turbo run build`, skipped when `VERCEL_ENV=preview` unless `RUN_PG_MIGRATE=true`)
- Concurrent migrators serialize with a PostgreSQL advisory lock
- Migrations are **skipped at runtime** (`NODE_ENV=production` / Vercel Preview) so Function cold starts stay cheap
- Faster startup (migrations already applied)
- Fails fast if migrations have issues
- Works with any deployment platform

**PostgreSQL (local development):**
- Fastify and eve call `runMigrations()` on boot. Development applies Drizzle SQL; production skip remains.
- Identity seed runs on API boot when `assets` is empty (`seedIdentity` in `apps/api`).
...
- **`PGLITE=false`** (default): Uses PostgreSQL via `POSTGRES_URL`
- When `PGLITE=true`, `POSTGRES_URL` is optional (PGLite ignores the connection string)
- When `PGLITE=false`, `POSTGRES_URL` is required
- Parse `PGLITE` with `parseBool` (`1` / `true` / `yes` / `on`). Do not use `z.coerce.boolean()` — the string `"false"` would enable PGLite.
...
flowchart TD
    ...
    UsePGLite --> RuntimeMigrate[PGLite migrations at runtime]
    RuntimeMigrate --> CreateInstance[Create PGLite Instance]
    CreateInstance --> AppReady[App Ready]

    UsePostgres --> RuntimeOrBuild{NODE_ENV production?}
    RuntimeOrBuild -->|no| DevMigrate[runMigrations on boot]
    RuntimeOrBuild -->|yes| MigratePhase[pnpm db:migrate]
    DevMigrate --> ConnectPG[Connect to PostgreSQL]
    MigratePhase --> ConnectPG
    ConnectPG --> AppReady

    MigratePhase -.->|Separate from pnpm build| Deploy[Docker / Railway / Fly.io / Render / local]
    Deploy --> MigratePhase
```
...
- **Docker**: `RUN pnpm build` (OpenAPI + `tsc`; run `pnpm db:migrate` as a separate migrate phase)
- **Railway/Fly.io/Render**: `build: pnpm build` then a migrate command — not bundled into `tsc`
- **Local dev (PostgreSQL)**: Fastify/eve `runMigrations()` on boot. `pnpm db:migrate` remains the Vercel/CLI path. Wipe with **`pnpm reset`**. **PGLite**: SQL assets must be present next to compiled `migrate.js`; empty list is a hard error when `PGLITE=true` or `NODE_ENV=test`

### Local database reset (Supabase + Drizzle)

For local Postgres via Supabase CLI (`project_id: basilic-api` in `packages/db/supabase/config.toml`), **`pnpm db:start`** then **`pnpm dev`** applies schema on boot. **`pnpm reset`** from the repo root (runs `pnpm --filter @repo/api reset`) is a wipe:

1. **`supabase db reset`** (`@repo/db` `db:reset`) — Drops and recreates the database. `[db.seed]` is off; no `seed.sql`.
2. **Drizzle migrations** — `scripts/migrate.ts` via `pnpm db:migrate` with `RUN_PG_MIGRATE=true` and the local Supabase `POSTGRES_URL`.
3. **Data seed** — `scripts/seed.ts` (`runSeed`). Same identity rows Fastify seeds on boot when `assets` is empty.
...
- **Separate PostgreSQL migrations**: run `pnpm db:migrate` as its own step (Docker migrate phase, Railway/Fly.io/Render, local). Not part of `pnpm build` / `tsc`
...
- **Strategic approach**: Basilic ships Vercel + Postgres (`POSTGRES_URL`). Other hosts (Supabase, GCP, AWS, on-prem) remain an adopter choice via the connection string, not a Basilic destination.
...
- [API Development](/docs/architecture/api) - PostgreSQL and Drizzle ORM integration
...
- [Installation](/docs/development) - Database configuration and connection setup
