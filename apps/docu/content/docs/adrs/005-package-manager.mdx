...
- **Runtime**: Local and CI use **Node.js 24.20.0 (LTS Krypton)** via `.nvmrc`. Package `engines.node` is `24.x` so hosts that only ship a Node major (for example Vercel) can install. Package management is pnpm.
