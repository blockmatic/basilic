...
Status: **Accepted** for shadcn + Tailwind. Primitive library updated in [ADR 013](/docs/adrs/013-shadcn-base-ui) (Base UI, `base-vega`).
...
- Built on Radix UI primitives (accessible by default) — later replaced by Base UI in [ADR 013](/docs/adrs/013-shadcn-base-ui)
...
| **Accessibility** | ✅ Headless primitives (now Base UI) | ✅ Good | ✅ Radix primitives | ⚠️ Manual |
...
- Copy-paste shadcn wrappers on accessible primitives (Base UI / `base-vega` as of [ADR 013](/docs/adrs/013-shadcn-base-ui)) with well-tested components
