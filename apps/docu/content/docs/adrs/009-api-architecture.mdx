...
The public agent contract on this scored host remains OpenAPI plus `@repo/core`. Durable agents run on **eve**, a separate host ([ADR 014](/docs/adrs/014-fastify-eve-vercel-runtime), [Eve](/docs/architecture/eve)). Do not advertise `/eve/` or `/mcp` from `llms.txt`. Eve is not in the tree until E1. Product MCP is not this change.
...
**MCP:** This ADR listed “AI/MCP integration” as a REST+OpenAPI benefit. Basilic does **not** ship an API MCP server. Agents use the OpenAPI spec, `@repo/core`, and `@repo/cli` (`basilic ai chat`). Cursor MCP in `.cursor/mcp.json` is IDE tooling (GitHub, CodeRabbit, shadcn, Vercel), not the product API.
...
- [Eve](/docs/architecture/eve) - durable agent runtime (queued until E1)
