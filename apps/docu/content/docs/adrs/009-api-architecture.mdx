...
The public agent contract on this scored host remains OpenAPI plus `@repo/core`. Durable agents run on **eve**, a separate host ([ADR 014](/docs/adrs/014-fastify-eve-vercel-runtime), [Eve](/docs/architecture/eve)). Do not advertise `/eve/` or `/mcp` from `llms.txt`. Product `command` and `chat` live in `apps/agents`. Product MCP is not this change.
...
- [Eve](/docs/architecture/eve) - durable agent runtime (`command` and `chat` in `apps/agents`)
