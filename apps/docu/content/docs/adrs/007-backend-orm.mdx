...
Schemas are defined using Drizzle's declarative syntax in `apps/api/src/db/schema/`:
...
- Drizzle instance with schema exported from `apps/api/src/db/schema/`
...
- Schema files live in `apps/api/src/db/schema/` directory
- Migrations are stored in `apps/api/src/db/migrations/`
