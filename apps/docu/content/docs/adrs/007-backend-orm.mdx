...
- **No proprietary APIs**: Standard `POSTGRES_URL`, no platform-specific code
...
- **No proprietary APIs**: Standard `POSTGRES_URL`, no platform-specific code
- **Security flexibility**: Can migrate to environments with KMS encryption, VPC isolation, HSMs
- **Chosen for portability**: SQL + Drizzle stay host-agnostic. Basilic’s shipped path is Vercel + `POSTGRES_URL` Postgres; GCP/AWS remain unresolved adopter choices.

## Implementation Details

### Schema Definition

Schemas are defined using Drizzle's declarative syntax in `packages/db/src/schema/`:
...
- Drizzle instance with schema exported from `@repo/db/schema`
- Schema exports for type inference
- Standard `POSTGRES_URL` connection string
...
- Schema files live in `packages/db/src/schema/` (`@repo/db/schema`)
- Migrations are stored in `packages/db/src/migrations/`
- Drizzle configuration in `packages/db/drizzle.config.ts`
- Fastify (`apps/api`) calls `configureDb` and `runMigrations`; encrypt helpers stay in api
...
- **Portability**: Migration between database hosts requires only changing `POSTGRES_URL`
- **Complements Fastify**: Both stay portable; Basilic ships on Vercel. GCP/AWS are adopter options, not a Basilic destination.

## Related Documentation

- [API Development](/docs/architecture/api) - Drizzle ORM integration with Fastify and PostgreSQL
