...
| `apps/agents` | Exclude the eve hello workspace until a later E PR copies it. |
...
| First-party extras | Strip `_first/` and `.agents/skills/f/`. Do not write a root `PRODUCT.md`. |
...
Generated projects should not inherit Basilic's roadmap as their own. Assembly must not copy `_first/` or `.agents/skills/f/`.
...
6. Print the destination and existing setup sequence: `pnpm setup`, database start, then `pnpm dev`, with a link to Product Ready. Do not automatically reset databases, create remotes, publish, deploy, or commit. Installation/setup remains an explicit next step in v1.
...
Extend the existing shared review workflow before introducing another agent service. At PR time, ask AI to compare the diff with public CLI/template contracts and suggest a conventional title, release impact, migration notes, and evidence paths. Run automatic analysis on every relevant PR if desired, but label the result advisory.
...
At launch, update root and generator READMEs, Getting Started, After fork, CLI, Publishing, GitHub Actions, and Monorepo documentation together. Keep forks documented for upstream contributions; new products use the generator. Existing copies remain owner-managed: offer changelogs and migration guidance before considering a future upgrade command.
