...
| Product brief | Write an unfilled root `PRODUCT.md`. Do not copy Basilic’s feature map or fabricate product decisions. |
...
Generated projects should not inherit Basilic's roadmap as their own. Assembly writes an unfilled root `PRODUCT.md` and must not copy `_first/` or `.agents/skills/f/`.
...
At launch, update root and generator READMEs, Getting Started, After fork, CLI, Publishing, GitHub Actions, and Monorepo documentation together. Update root `PRODUCT.md` with the delivered adoption capability. Keep forks documented for upstream contributions; new products use the generator. Existing copies remain owner-managed: offer changelogs and migration guidance before considering a future upgrade command.
