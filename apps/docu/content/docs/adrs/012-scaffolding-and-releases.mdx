...
description: "Proposed Basilic generator, template boundaries, conventional-commit versioning, and AI-assisted releases."
---

Status: **Proposed — implementation plan, not shipped behavior.** September 5, 2026.
...
- “MPX … init” means an `npx`-style project creation command. Examples below describe the proposed interface and are not available yet.
...
| Root README and After fork describe GitHub template/fork adoption | CLI onboarding is new; current instructions remain valid until launch. |
...
| Checked-in workflows cover quality, security, and mobile delivery | No Release Please, Changesets, or npm release workflow was found. Conventional commits are documented; no dedicated commit-message gate was found. |
...
| FIRST user pack | Retain reusable guidance/templates; replace `_first/basilic` with an unfilled adopter instance and retarget `FIRST.md` and agent pointers. Do not fabricate product decisions. |
...
**Decisions remaining before implementation:** verified npm identity/owner; acceptance of the lightweight docs snapshot and FIRST reset; stable-release baseline and preview need; maintainers authorized to approve releases. These are review items, not claims that accounts or infrastructure have already been configured.
