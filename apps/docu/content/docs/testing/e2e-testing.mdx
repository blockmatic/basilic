...
| `wallet-metamask-auth.spec.ts` | EVM wallet sign-in via mock |
| `wallet-solana-auth.spec.ts` | Solana wallet sign-in via mock |
| `link-email.spec.ts` | Link email section after wallet login; "already linked" when user has email (magic link) |
...
flowchart TB
  ...
  MagicLinkAuth --> WalletAuth --> LinkWallet --> WalletMetaMask --> WalletSolana --> LinkEmail --> ChromiumProj
```
