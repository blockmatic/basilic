...
Single **E2E workflow** (`next-e2e-test.yml`) triggers on `pull_request`. Uses a matrix strategy to run Next and Fastify E2E in parallel when path filters match. Resolves project IDs from project names (`basilic-next`, `basilic-fastify`) via the Vercel API. Matches deployments by `meta.githubCommitRef` (PR branch). Waits 5 min for Vercel to start builds, then polls up to 30 times (15s apart). Fails with a descriptive error if no deployment is found. Runs `pnpm --filter @repo/next test:e2e` with `PLAYWRIGHT_APP_URL` and `PLAYWRIGHT_API_URL`, or `pnpm --filter @repo/fastify test:e2e` with `PLAYWRIGHT_API_URL`.

Requires `VERCEL_TOKEN`, `VERCEL_TEAM_ID` (repository variable), and `VERCEL_AUTOMATION_BYPASS_SECRET` for deployment protection bypass.
...
- **E2E CI (Vercel previews)**: Require `VERCEL_TOKEN` (from [vercel.com/account/tokens](https://vercel.com/account/tokens)), `VERCEL_TEAM_ID` (repository variable), and `VERCEL_AUTOMATION_BYPASS_SECRET`. Workflow polls the Vercel API for deployment URLs matching the PR branch (`meta.githubCommitRef`).
