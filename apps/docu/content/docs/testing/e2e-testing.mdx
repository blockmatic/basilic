...
The `next-deploy-e2e.yml` and `api-deploy-e2e.yml` workflows run on pull requests (open and sync). After deploy completes:
...
The deploy job builds and deploys to Vercel via CLI; URLs are parsed and passed to E2E.
...
| `chat-assistant.spec.ts` | Chat UI with authenticatedPage; Who am I? prompt, assistant response, user info card |
...
wallet-metamask-auth, wallet-solana-auth, wallet-auth, link-wallet, link-email are excluded from runs (testIgnore). Badges spec was removed.
