...
| `magic-link-verified.spec.ts` | Profile shows verified email state after magic link auth via `/auth/login` |
...
flowchart TB
  subgraph AuthProj [Auth project]
    ...
    MagicLinkVerified[magic-link-verified.spec.ts]
  end
  ...
  MagicLinkAuth --> MagicLinkVerified --> ChromiumProj
```
