...
Wallet-related E2E specs (`wallet-auth.spec.ts`, `link-wallet.spec.ts`, `link-email.spec.ts`) run without browser extensions. They verify login page wallet buttons, dashboard navigation, link wallet/email UI visibility, and sign-out.

**Full wallet flows** (sign-in with MetaMask/Phantom, link wallet) require Playwright with extension loading: install MetaMask or Phantom as a test fixture, seed a test account, and configure the browser context. See [Playwright browser contexts](https://playwright.dev/docs/browser-contexts) for extension setup. Test account seeding typically uses a known mnemonic or imported key for deterministic addresses.
