...
Two workflows trigger on Vercel `repository_dispatch` when a PR preview deployment succeeds:

- **next-e2e-test.yml** — Runs when `basilic-next` PR preview deploys. Uses deployment URL from webhook; derives API URL from `git.ref` (`https://basilic-fastify-git-{branch}-gaboesquivel.vercel.app`). Runs `pnpm --filter @repo/next test:e2e`.
- **api-e2e-test.yml** — Runs when `basilic-fastify` PR preview deploys. Uses deployment URL from payload. Runs `pnpm --filter @repo/fastify test:e2e`.

Workflow files must exist on `main`. The `target == null` condition ensures tests run only for PR preview deployments, not production. Filter by `projectId` or `project.name` (replace `prj_0` with your Vercel project ID from Project Settings → General). Uses `vercel/repository-dispatch/actions/checkout` to checkout the deployment commit.

Requires `VERCEL_AUTOMATION_BYPASS_SECRET` for deployment protection bypass.
