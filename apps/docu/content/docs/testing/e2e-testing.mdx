...
Chat E2E needs `ANTHROPIC_API_KEY` (in-app Claude Haiku 4.5 by default). CI and `test:e2e:local` set `AI_PROVIDER=anthropic`. Web chat (`chat-assistant.spec.ts`) skips only on provider infra failures: 402/quota markers, 401/invalid API key, network/timeout/522 errors, and upstream/5xx messages — not on optional UI like `user-info-card`. API remote AI tests (`ai.spec.ts` → `chat.test.ts`, `generate.test.ts`) delegate skip checks to `test/utils/ai-remote.ts`: 402 responses with credit/quota markers, plus 502/503/504 and connection failures.
...
| `GET /test/magic-link/last?email=` | Optional email filter; omit email for latest `@test.ai` (Scalar E2E) |
...
Ports in use: `bash scripts/kill-test-servers.sh`. Token extraction fails: confirm `ALLOW_TEST=true` and the correct `@test.ai` email query on `/test/verification/last`. Magic link `500` mid-suite: local/CI spawn sets `RATE_LIMIT_MAX=10000` (default API limit is 100/min per IP).
