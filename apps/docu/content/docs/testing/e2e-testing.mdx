...
Playwright specs are currently skipped (`test.describe.skip`) because the Chromium headless shell is not installed locally. Re-enable after `pnpm exec playwright install`, then remove `.skip` from the describe blocks.
