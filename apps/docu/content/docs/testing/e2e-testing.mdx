...
The `next-e2e.yml` and `api-e2e.yml` workflows run on pull requests (open and sync). Both use `test:e2e:local`—no Vercel deploys. Build once, then E2E against local servers.
