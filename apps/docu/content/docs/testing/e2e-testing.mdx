...
- **Next CI**: Uses `tj-actions/vercel-wait` to wait for Vercel Fastify and Next deployments, runs `pnpm --filter @repo/next test:e2e` against deployed URLs. Requires `VERCEL_TOKEN`, `VERCEL_PROJECT_ID`, `VERCEL_NEXT_PROJECT_ID`, and `VERCEL_AUTOMATION_BYPASS_SECRET`.
- **API CI**: Uses `tj-actions/vercel-wait` to wait for Vercel Fastify deployment, runs `pnpm --filter @repo/fastify test:e2e` against the deployed API. Requires `VERCEL_TOKEN`, `VERCEL_PROJECT_ID` (and `VERCEL_TEAM_ID` for team projects), `VERCEL_AUTOMATION_BYPASS_SECRET`.
...
- **API CI (Vercel E2E)**: Requires `VERCEL_TOKEN` (from [vercel.com/account/tokens](https://vercel.com/account/tokens)), `VERCEL_PROJECT_ID` (basilic-fastify), optionally `VERCEL_TEAM_ID` for team projects, and `VERCEL_AUTOMATION_BYPASS_SECRET`.
- **Next CI (Vercel E2E)**: Same plus `VERCEL_NEXT_PROJECT_ID` (basilic-next project settings).
