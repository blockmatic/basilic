...
- **Next CI**: `pnpm --filter @repo/next test:e2e:local` — builds, spawns Fastify + Next with PGLite, runs Playwright, uploads report on failure.
- **API CI**: Waits for Vercel Fastify deployment (branch preview URL), runs `pnpm --filter @repo/fastify test:e2e` against the deployed API, uploads report on failure. Requires `VERCEL_AUTOMATION_BYPASS_SECRET`.
...
- **API CI (Vercel E2E)**: Requires `VERCEL_AUTOMATION_BYPASS_SECRET` in GitHub secrets to bypass deployment protection when polling and running Playwright against preview URLs.
