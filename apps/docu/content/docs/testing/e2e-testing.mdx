...
Chat E2E needs `ANTHROPIC_API_KEY` (in-app Claude Haiku 4.5 by default). CI and `test:e2e:local` set `AI_PROVIDER=anthropic`. Chat skips only on provider infra failures (401/402/network/5xx), not on optional UI like `user-info-card`.
