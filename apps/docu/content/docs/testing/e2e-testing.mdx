...
- **Next CI**: Triggers on `pull_request` and `push` to main. Polls Vercel API for basilic-next and basilic-fastify deployment URLs (every 45s, up to 18 min). When both are ready, runs `pnpm --filter @repo/next test:e2e` with `PLAYWRIGHT_APP_URL` and `PLAYWRIGHT_API_URL`. When builds are ignored (turbo-ignore), workflow skips with success so PRs are not blocked.
- **API CI**: Same trigger. Polls for basilic-fastify deployment, runs `pnpm --filter @repo/fastify test:e2e`.

Both workflows require `VERCEL_TOKEN`, `VERCEL_TEAM_SLUG` (or `VERCEL_TEAM_ID`), and `VERCEL_AUTOMATION_BYPASS_SECRET` for deployment protection bypass.
...
- **API CI and Next CI (Vercel E2E)**: Require `VERCEL_TOKEN` (from [vercel.com/account/tokens](https://vercel.com/account/tokens)), `VERCEL_TEAM_SLUG` (team slug, e.g. "gaboesquivel", for team projects; or `VERCEL_TEAM_ID`), and `VERCEL_AUTOMATION_BYPASS_SECRET` for deployment protection bypass. Workflows poll the Vercel API for deployment URLs by commit SHA.
...
| `wallet-metamask-auth.spec.ts` | Full MetaMask login flow (Synpress): connect, sign SIWE, redirect to / |
| `wallet-solana-auth.spec.ts` | Full Solana/Phantom login flow (Synpress): connect, sign SIWS, redirect to / |
...
### Full wallet flows (Synpress)

`wallet-metamask-auth.spec.ts` and `wallet-solana-auth.spec.ts` use **Synpress** to inject real MetaMask and Phantom extensions. No mocks; tests perform actual connect + sign flows.

**Setup:**
1. Build wallet cache before running: `pnpm synpress:build` (or `pnpm synpress:metamask` and `pnpm synpress:phantom` separately)
2. Cache stored in `.cache-synpress` (gitignored)
3. `test:e2e:local` runs Synpress cache build automatically before Playwright
4. On Linux CI (no DISPLAY): Synpress cache build and Playwright run use `xvfb-run`; MetaMask has known headless compatibility issues
5. Phantom cache build may fail when `crx-backup.phantom.dev` is unreachable; CI soft-fails and skips only wallet-solana tests
6. Root `pnpm test:e2e` skips wallet specs (`SKIP_WALLET_E2E=1`); run `pnpm -F @repo/next test:e2e:local` to include them
7. Wallet setup files: `e2e/wallet-setup/metamask.setup.ts`, `e2e/wallet-setup/phantom.setup.ts` (seed: standard test phrase)

**Projects:** `wallet-metamask` and `wallet-solana` run in separate Playwright projects. Synpress fixtures (`metaMaskFixtures`, `phantomFixtures`) provide extension-injected context.
