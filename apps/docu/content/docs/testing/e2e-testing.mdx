...
| `pnpm --filter @repo/api test:e2e:local` | Spawn API (`RATE_LIMIT_MAX=10000`, `COINS_RATE_LIMIT_MAX=10000`), run Scalar login E2E — no `ANTHROPIC_API_KEY` required |
| `pnpm test:e2e:ui` / `test:e2e:debug` | Playwright UI or debug (in the app) |

Pass URLs with `--app` / `--api`, or `PLAYWRIGHT_APP_URL` / `PLAYWRIGHT_API_URL`. Spawned local/CI suites still default to `http://localhost:3000` and `http://localhost:3001` — no Portless, no eve process. `scripts/e2e-local-shared.mjs` pins `EVE_COMMAND_URL` / `EVE_CHAT_URL` to `http://127.0.0.1:3004` / `:3005` so Fastify `GET /agents` does not advertise `https://agents.basilic.localhost` (that host is not listening in Playwright, and the rail would hang with Send disabled). Against a running `pnpm dev`, use `https://basilic.localhost` and `https://api.basilic.localhost` (eve mounts stay on `https://agents.basilic.localhost/eve/command` and `/eve/chat`; the current Playwright DAG does not hit them).
...
Use **one worker** (PGlite cannot take concurrent writers). The setup project logs in as `test@test.ai` once and writes `test-results/.auth/user.json`. **chromium** and **security** reuse that file via project `storageState`. **auth**, **passkey-login**, and logout tests use empty storage so they never revoke the shared session.
...
API Scalar E2E (`pnpm --filter @repo/api test:e2e:local`) does not call AI and does not require Anthropic. API remote tests (`ai.spec.ts`) use `test/utils/ai-remote.ts`: skip 402 always; skip 502/503/504 only with placeholder/missing key — real key fails on 502. See [AI Architecture](/docs/architecture/ai).
...
4. **chromium** — dashboard smokes, coin-board chips, 404 (storageState)
...
| `dashboard.spec.ts` | chromium | Coin board (fixture USD, sort, watch, chips, sidebar/rail chrome, `q`, Share, `surface=account`, `surface=dashboard`, Who am I?, stubbed missing SpeechRecognition hides mic), profile, 404 |
...
Ports in use: `bash scripts/kill-test-servers.sh`. Token extraction fails: confirm `ALLOW_TEST=true` and the correct `@test.ai` email query on `/test/verification/last`. Magic link `500` mid-suite: local/CI spawn sets `RATE_LIMIT_MAX=10000` and `COINS_RATE_LIMIT_MAX=10000` (default API limit is 100/min per IP; `GET /coins` defaults to 10). Spawn sets `COINS_USE_FIXTURE=true` so the coin board uses fixture quotes (Doge first at `?sortBy=change24h&sortDir=desc`). Passkey registration fails in CI: spawn scripts and `web-e2e.yml` set `WEBAUTHN_RP_NAME=Test App` when `.env.test` is absent.
