...
- **Tools**: Playwright
- **Commands**: `pnpm test:e2e:local` (spawn servers) or `pnpm test:e2e` (URLs from env/params)
...
- **Chat E2E** — Disabled for now (`test.describe.skip`); to be tested in a later iteration
...
See [apps/next/README.md](../../../../apps/next/README.md) for test scripts (test, test:watch, test:ui, test:e2e, test:cov).
...
- **E2E Tests**: Never mock — E2E tests use real Fastify and Next.js servers (started via `pnpm test:e2e:local` in CI or manually)
