...
- **AI remote** — API Vitest (`ai.spec.ts`) skips 402 always. Skips 502/503/504/connection only with a missing or placeholder `ANTHROPIC_API_KEY` (`sk-ant-xxx`); a real key fails on 502. Web chat E2E (`chat-assistant.spec.ts`) still soft-passes on 402/upstream/502. Default model: `claude-haiku-4-5` (alias `sonnet` → `claude-sonnet-4-6`). See [AI Architecture](/docs/architecture/ai).
