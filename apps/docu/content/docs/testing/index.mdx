...
When `ALLOW_TEST=true` (non-production), magic-link flows for `*@test.ai` store plain tokens in `verification.token_plain`. E2E reads them via `/test/magic-link/last`. Do not enable on production deployments.
