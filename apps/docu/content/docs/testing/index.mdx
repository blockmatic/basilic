...
Vitest pins `ALLOWED_ORIGINS` to `http://localhost:3000,http://127.0.0.1:3000,https://example.com` when unset so passkey origin tests are not no-ops under `*`.
...
- **AI remote** — Skips (not passes) on 402 or placeholder-key upstream errors. Default model: `claude-haiku-4-5` (alias `sonnet` → `claude-sonnet-4-6`). See [AI Architecture](/docs/architecture/ai).
