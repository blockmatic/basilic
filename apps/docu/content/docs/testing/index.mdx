...
When `ALLOW_TEST=true` (non-production), magic-link and change-email flows for `*@test.ai` store plain tokens in `verification.token_plain`. E2E reads them via `GET /test/verification/last` (email-scoped) or `GET /test/magic-link/last` (optional email). Do not enable on production deployments.
