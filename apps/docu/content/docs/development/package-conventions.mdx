...
CoinGecko + Binance HTTP lives in `apps/api/src/lib/markets/` until extract to `@repo/markets`. Zero Fastify, zero `@repo/db`, zero Alchemy. Named exports: `searchAssets`, `getMarkets`, `getQuote`, `getCandles`, `getTrending`, `getAsset`, `getGlobal`. Next.js must not import this folder.
