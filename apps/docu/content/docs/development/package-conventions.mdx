...
The server app (`apps/api`) should treat **Fastify routes + TypeBox schemas** as the source of truth.

1. Define request/response schemas in the route (TypeBox)
2. Generate OpenAPI from routes into `apps/api/openapi/openapi.json`
