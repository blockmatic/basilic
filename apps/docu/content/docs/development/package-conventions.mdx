...
CoinGecko Demo + Binance public REST. Zero Fastify, zero `@repo/db`, zero Alchemy. Hosts call `configureMarkets` from their env. Named exports: `searchAssets`, `getMarkets`, `getQuote`, `getCandles`, `getTrending`, `getAsset`, `getGlobal`. Fastify `GET /coins` calls `getMarkets`. Eve command tools call the same fns. Next.js must not import this package.
