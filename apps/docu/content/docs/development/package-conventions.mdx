...
- `@repo/db` → Drizzle schema, `configureDb` / `getDb`, named fns (`listAssets`, watches, `getValidSession`). Fastify imports it. Eve hello (`apps/agents`) must not; named fns on the eve host land in E2. Next.js must not.
...
PostgreSQL access for Fastify (and the eve host from E2). Schema barrel `@repo/db/schema`, factory `configureDb` / `getDb`, PGLite `runMigrations` on `@repo/db/migrate`. Named RORO fns take `{ userId }` for user-scoped rows. Encrypt helpers and identity seed stay in `apps/api`.
...
### 5) Markets (collocated until E2)

CoinGecko + Binance HTTP lives in `apps/api/src/lib/markets/` until extract to `@repo/markets`. Zero Fastify, zero `@repo/db`, zero Alchemy. Named exports: `searchAssets`, `getMarkets`, `getQuote`, `getCandles`, `getTrending`, `getAsset`, `getGlobal`. Fastify `GET /coins` and `getMarketSnapshot` call `getMarkets`. Next.js must not import this folder.
