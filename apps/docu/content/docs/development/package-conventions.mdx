...
packages/
  ...
  db/          # Drizzle schema, factory, named data-access fns
  markets/     # CoinGecko + Binance vendor HTTP
  onchain/     # Alchemy Free Portfolio HTTP
apps/
...
- `@repo/db` → Drizzle schema, `configureDb` / `getDb`, named fns (`listAssets`, watches, `getValidSession`). Fastify and the eve host import it. Next.js must not.
- `@repo/markets` → CoinGecko + Binance HTTP. Fastify and eve command tools import it. Next.js must not.
- `@repo/onchain` → Alchemy Portfolio HTTP. Fastify and eve command tools import it. Next.js must not.

No reverse dependencies. `@repo/db` does not import Fastify env or the api test harness.
...
flowchart TB
  subgraph P[packages]
    ...
    D["@repo/db<br/>schema + named fns"]
    M["@repo/markets<br/>CoinGecko + Binance"]
    O["@repo/onchain<br/>Alchemy Portfolio"]
  end
...
* Served OpenAPI JSON at `/openapi.json` (`/reference/openapi.json` alias) and Scalar UI at `/reference`
...
PostgreSQL access for Fastify and the eve host. Schema barrel `@repo/db/schema`, factory `configureDb` / `getDb`, `runMigrations` on `@repo/db/migrate` (PGLite always; Postgres at boot in development). Local Docker is Supabase CLI in `packages/db/supabase/`. Named RORO fns take `{ userId }` for user-scoped rows. Encrypt helpers and identity seed stay in `apps/api`.

**Hard rules**

- No Fastify dependency and no import of api `env` or `test/utils/db`
- Next.js must not import `@repo/db` or `drizzle-orm` (eslint in `apps/web`)
- Do not add a second Vercel `db:migrate` filter; api still runs `turbo run db:migrate --filter=@repo/api`

### 5) `packages/markets`

CoinGecko Demo + Binance public REST. Zero Fastify, zero `@repo/db`, zero Alchemy. Hosts call `configureMarkets` from their env. Named exports: `searchAssets`, `getMarkets`, `getQuote`, `getCandles`, `getTrending`, `getAsset`, `getGlobal`. Fastify `GET /coins` calls `getMarkets`. Fastify `GET /coins/:assetId/candles` calls `getCandles` after a Binance `asset_markets` lookup in `@repo/db`. Eve command tools call the same fns. Next.js must not import this package.

### 6) `packages/onchain`

Alchemy Free Portfolio (`getWallet`, `getNfts`) for eth-mainnet and base-mainnet. Zero Fastify, zero `@repo/db`, zero `@repo/markets`. Hosts call `configureOnchain` from their env. Fastify `GET /account/wallet` and eve `get_wallet` / `get_nfts` compose linked `eip155` identity + this HTTP + `getQuote`. Holdings are not stored. Next.js must not import this package.
