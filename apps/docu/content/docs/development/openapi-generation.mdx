...
3. Generates OpenAPI 3.0 spec at `apps/api/openapi/openapi.json`
...
```bash
# Generate runtime-agnostic client and types
...
// Runtime-agnostic client (JWT mode for authenticated calls)
...
2. **Generate OpenAPI** - Run `pnpm --filter @repo/api generate:openapi`
3. **Generate clients** - Run `pnpm --filter @repo/core generate` (generates client and types; `@repo/react` consumes `@repo/core`)
4. **Use typed client** - Import and use in frontend with full type safety

## OpenAPI Spec Location

The generated OpenAPI spec is available at:
- **File**: `apps/api/openapi/openapi.json`
