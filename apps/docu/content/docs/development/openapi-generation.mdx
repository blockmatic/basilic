...
`@repo/web`, `@repo/docu`, and package CI builds use committed `openapi.json` and `src/gen/`. Regenerate after route changes. Every PR runs `pnpm openapi:drift` and `pnpm openapi:lint` in `lint.yml`; `api-e2e` repeats drift when API paths change. The API Vercel build still runs `generate:openapi` inside `@repo/api` `build`.

Redocly config: root `.redocly.yaml` (operationId uniqueness, path query segments; TypeBox null unions stay allowed via disabled `struct` / trailing-slash rules until the generator changes).

Live OpenAPI is `GET /openapi.json` (`/reference/openapi.json` is an alias); Scalar UI at `/reference`. Config: `packages/core/openapi-ts.config.ts`.
