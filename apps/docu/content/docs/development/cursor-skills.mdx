...
This repository should have one workflow tree. Slash tags are `/workflow` and `/plan-feature`. `/workflow plan` is a dispatcher shortcut to `/plan-feature`. After a refresh, remove any leftover `b` package with the skills CLI, preserving local edits first.
...
A named refresh is valid (`--skill next-v16`, `--skill workflow`). Do not keep a leftover local `b` tree beside `workflow`.
