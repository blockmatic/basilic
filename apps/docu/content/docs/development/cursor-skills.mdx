...
Non-interactive Cursor copy of the whole catalog:
...
The published workflow package is `workflow`: `.agents/skills/workflow/SKILL.md` dispatches to 51 children, including `/build` and `/use-frontend`. Shared authoring and completion references travel inside this tree. FIRST and technology skill names are unchanged.

This repository should have one workflow tree. Slash tags are `/workflow` and `/plan-feature`. `/workflow plan` is a dispatcher shortcut to `/plan-feature`. After a refresh, preserve local edits first, then run `pnpm dlx skills@latest remove --skill b -a cursor -y`. `.agents/skills/b/` must be absent afterward.
...
```bash
pnpm dlx skills@latest add blockmatic/basilic-skills --skill '*' -a cursor --copy -y
pnpm dlx skills@latest add blockmatic/first -a cursor --copy -y
```

A named refresh is valid (`--skill next-v16`, `--skill workflow`). After a refresh, run `pnpm dlx skills@latest remove --skill b -a cursor -y`. `.agents/skills/b/` must be absent afterward.
