...
That writes tech skills under `.agents/skills/<name>/` and playbooks under `.agents/skills/workflow/`. Full installer options: catalog [README](https://github.com/blockmatic/basilic-skills#agents-and-paths).

Root `skills-lock.json` tracks catalog hashes. The `web-design-guidelines-v1` entry also records the pinned `command.md` commit digest. Commit the lock with `.agents/skills/` after every refresh.
