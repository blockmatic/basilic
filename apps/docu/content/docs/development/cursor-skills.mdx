...
Root `skills-lock.json` tracks catalog hashes. FIRST may be `source: blockmatic/first` or a local sibling (`../first`) until that repo is published. Commit the lock with `.agents/skills/` after every refresh.
