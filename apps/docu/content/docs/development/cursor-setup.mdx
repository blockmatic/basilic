...
The portable contract is root `AGENTS.md` and `.agents/skills/` (installed by `pnpm setup`). Daily playbooks: [AI Workflow](/docs/development/ai-workflow). Catalog and skills CLI: [Skills](/docs/development/cursor-skills). Ports for any IDE: [Dev Environments](/docs/development/dev-environments).

The steps below are **Cursor extras** (Indexing & Docs, `.cursor/mcp.json`). Other agents skip them. Claude Code imports `AGENTS.md` from `CLAUDE.md`.
