...
| `@repo/ui` | Shared shadcn/ui components | A | `@repo/ui/components/*`, `@repo/ui/lib/*`, `@repo/ui/radix` |
...
| `@repo/notif` | Notification env/types (no root export) | B | `@repo/notif/node`, `@repo/notif/types/*` |
...
Handwritten hooks on `@repo/core` (not generated). Public exports include `ApiProvider`, `createReactApiConfig`, `useReactApiConfig`, session/user, magic link, OAuth, passkeys, TOTP, API keys, profile, chat, and Web3 verify (`useVerifyWeb3Auth`, `useVerifyLinkWallet`, `useLinkEmail`). See `packages/react/src/index.ts`.
