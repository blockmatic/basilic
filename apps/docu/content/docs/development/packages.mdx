...
Handwritten hooks on `@repo/core` (not generated). Public exports include `ApiProvider`, `createReactApiConfig`, `useReactApiConfig`, session/user, magic link, OAuth, passkeys, TOTP, API keys, profile, `useChatFromConfig` (in-shell [demo assistant](/docs/architecture/ai)), and Web3 verify (`useVerifyWeb3Auth`, `useVerifyLinkWallet`, `useLinkEmail`). See `packages/react/src/index.ts`.
