...
| Remote + other IDE | Forward 3000, 3001, 3004, 3005; run `pnpm dev` | Forward 8081 + `start:localhost`, or use `start:tunnel` |
...
| 3004 | eve command | Root `pnpm dev` TUI pane `@repo/agents#dev`. Health `GET /eve/v1/health`. |
| 3005 | eve chat | Root `pnpm dev` TUI pane `@repo/agents#eve:dev:chat`. Health `GET /eve/v1/health`. |
...
Ensures local Postgres is up, then starts Turborepo's TUI (same left-pane logs as `pnpm lint:eslint` / `pnpm test`): Fastify, Next.js, eve command/chat in this repo, and dependent package watchers. Uses LAN addresses by default. Mobile stays a separate command. Generated projects omit agents.
...
1. Ensure ports `3000`, `3001`, `3004`, `3005`, `8081` (and optionally `19000`, `19001`) are forwarded. Cursor does this automatically; in VS Code, check the Ports view.
...
These run where you execute `pnpm dev`. Browsers reach them via `localhost` (when forwarded) or VPC IP (when on VPN). Local Postgres is Docker via `pnpm db:start` (Supabase CLI in `apps/api`); `@repo/db` only rebuilds schema `dist/`.
