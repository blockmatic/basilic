...
`pnpm setup` / `pnpm setup:portless` trusts the local CA and starts the proxy on **443** (may prompt for sudo once). Re-run is safe. CI skips this step. After reboot, run `pnpm exec portless proxy start` or install `portless service install`. `portless get` may print `:1355` if the HTTPS proxy was never started; Basilic env still uses `https://<name>.localhost` (port 443).
