...
These run where you execute `pnpm dev`. Browsers reach them via `localhost` (when forwarded) or VPC IP (when on VPN). Local Postgres is Docker via `pnpm db:start` (Supabase CLI in `packages/db`).
