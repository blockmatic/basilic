...
Install Node.js 24.20.0 (LTS Krypton) with nvm. Local and CI pin this exact version in `.nvmrc` and `.node-version`. Package `engines.node` is `24.x` so hosts that only ship a Node major (for example Vercel) can install.
