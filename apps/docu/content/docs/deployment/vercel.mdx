...
- **fastify** — Uses `scripts/vercel-ignore.sh` with custom paths including `apps/next` (see Branch Preview Coupling below)

`turbo-ignore` and the fastify script use `--fallback=HEAD~10` / `HEAD~10` when `VERCEL_GIT_PREVIOUS_SHA` is empty (first deploy, PR preview), avoiding unnecessary builds.

## Branch Preview Coupling

Next's `apps/next/next.config.mjs` computes `NEXT_PUBLIC_API_URL` for preview deployments as `basilic-fastify-git-{branch}-*.vercel.app`. A Next branch preview expects a matching Fastify branch deployment for that same branch. If Fastify skipped when only Next changes, the preview would point at a non-existent API URL.

**Fastify must build every time Next changes** so branch instances can connect. The Fastify `ignoreCommand` includes `apps/next` in its diff paths for this reason.
