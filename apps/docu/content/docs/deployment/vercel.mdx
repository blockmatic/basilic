...
**API (`apps/api`)** runs compile and OpenAPI generation via `turbo run build`, then database migrations via a separate `db:migrate` task:

```json
{
  "buildCommand": "cd ../.. && pnpm turbo run build db:migrate --filter=@repo/api..."
}
```

Migrations run against real Postgres on Vercel; they no-op under `PGLITE` or `NODE_ENV=test` (CI E2E).
