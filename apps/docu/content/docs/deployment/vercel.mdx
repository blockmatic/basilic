...
- **Path duplication**: The workflow automatically sets `rootDirectory` to `null` via the Vercel API before each deploy. If you still see path duplication, ensure Vercel project settings → General → Root Directory is `.` or blank.
