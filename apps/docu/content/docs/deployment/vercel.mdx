...
Git deployments are enabled. Vercel automatically builds and deploys on push: preview environments for PRs and branches, production for main/develop. CI workflows (next-e2e, api-e2e, docu-build) run tests and build validation locally; they do not trigger deploys.
...
Each app uses `ignoreCommand` in `vercel.json` (e.g. `turbo-ignore @repo/next`) so Vercel skips deploys when only unrelated paths change. Vercel builds only when relevant code changes.
