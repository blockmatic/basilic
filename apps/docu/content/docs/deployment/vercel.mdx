...
- **pnpm ENOENT / wrapper missing**: Vercel’s `packageManager` provisioning skips lifecycle scripts, so pnpm 12’s native binary never materializes. `vercel.json` `installCommand` runs `node scripts/vercel-install.mjs` (`npm install -g pnpm@<pin> --foreground-scripts`, then `pnpm install`).
