...
Automatic Git deployments are disabled (`git.deploymentEnabled: false`). Per-app workflows (`next-deploy-e2e.yml`, `api-deploy-e2e.yml`, `docu-deploy.yml`) deploy after unit tests pass (or directly for docu) on every push and pull request. Main/develop use production env; other branches use preview.
