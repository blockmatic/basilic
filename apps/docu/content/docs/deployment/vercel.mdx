...
- **docu** — Builds when `apps/docu`, `packages/ui`, `package.json`, or `pnpm-lock.yaml` change
- **next** — Builds when `apps/next`, `packages`, `package.json`, or `pnpm-lock.yaml` change
- **fastify** — Builds when `apps/fastify`, `apps/next`, or their dependencies change (see Branch Preview Coupling below)
