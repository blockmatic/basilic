...
Automatic Git deployments are disabled (`git.deploymentEnabled: false`). The `deploy-and-e2e.yml` workflow deploys after unit tests pass on every push and pull request. Main/develop use production env; other branches use preview.
...
- Check `deploy-and-e2e.yml` workflow logs
