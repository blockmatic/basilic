...
By default, the app uses **Open Router** with `meta-llama/llama-3.3-70b-instruct:free`. We recommend **self-hosting** with Ollama when data privacy is a priority—prompts and responses stay on your infrastructure, with no third-party API calls.
...
When Ollama is used as the CI AI provider (e.g. `OLLAMA_BASE_URL` secret in GitHub Actions), `api-e2e` and `next-e2e` can run in parallel and both hit the same server. With `OLLAMA_NUM_PARALLEL=1`, requests serialize and tests may time out.
...
The Fastify API uses **Open Router** as the default when `OPEN_ROUTER_API_KEY` is set (`meta-llama/llama-3.3-70b-instruct:free`). Privacy-first routing with Ollama requires both `OLLAMA_BASE_URL` and `AI_PROVIDER=ollama` when `OPEN_ROUTER_API_KEY` exists—otherwise `getResolvedProvider()` prefers Open Router. See the provider selection logic in `getResolvedProvider()` for the exact precedence.
...
At least one of `OPEN_ROUTER_API_KEY` or `OLLAMA_BASE_URL` must be set; `OLLAMA_BASE_URL` is used for self-hosted/private setups.

| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `OPEN_ROUTER_API_KEY` | Conditional | — | Open Router API key (get at openrouter.ai) |
| `OLLAMA_BASE_URL` | Conditional | `http://localhost:11434` | Ollama server URL when self-hosting |
| `AI_PROVIDER` | Conditional | Inferred | `openrouter` or `ollama` — inferred when only one provider configured; must be set when both vars present |
| `AI_DEFAULT_MODEL` | No | `meta-llama/llama-3.3-70b-instruct:free` (OR) / `qwen3:8b` (Ollama) | Default model |

### Provider Selection

1. If `AI_PROVIDER` is set, use it (requires the corresponding env var).
2. Else if `OPEN_ROUTER_API_KEY` is set, use Open Router.
3. Else if `OLLAMA_BASE_URL` is set, use Ollama.
