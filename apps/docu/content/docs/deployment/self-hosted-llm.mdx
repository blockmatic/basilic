...
This guide covers [Ollama](https://ollama.com)—a local inference runtime with an OpenAI-compatible **wire** API—for development and production. Basilic does **not** ship a first-class OpenAI SDK; in-app chat uses Anthropic → OpenRouter → Ollama. OpenAI-compatible HTTP is not the OpenAI SDK.

## Why Self-Host for Data Privacy

| Concern | Cloud APIs (Anthropic, OpenRouter, …) | Self-Hosted (Ollama) |
|--------|-------------------------------------|----------------------|
| Data leaves your infra | Yes—sent to vendor | No—stays local |
| Vendor retention | Varies by ToS | None |
| Data residency | Vendor regions | You choose where it runs |
| Cost at scale | Per-token pricing | Fixed (hardware) |
| Latency | Network round-trip | Local inference |

**Use self-hosted when:** prompts should stay on your machine or VPC. Use cloud APIs for rapid prototyping or frontier models. This starter does not claim HIPAA or SOC 2 certification.
...
The Fastify API supports **Anthropic** (default), **Open Router**, and **Ollama**. Routes, env vars, streaming, tools, and the web assistant contract are documented in [AI Architecture](/docs/architecture/ai).

Privacy-first routing with Ollama requires `AI_PROVIDER=ollama` when other keys exist.

### Tool support (Ollama)

Open Router and capable Ollama models support tools (`getAccountInfo`, `getMarketSnapshot`, optional `braveSearch`). Small Ollama models (e.g. `qwen2.5:3b`) may not; use 7B+ such as `llama3.1:8b` or the runtime default `qwen3:8b` for reliable tool calling.

---

## Related Documentation

- [AI Architecture](/docs/architecture/ai) — Routes, env table, `lib/ai`, web contract
