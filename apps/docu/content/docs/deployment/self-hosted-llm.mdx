...
The Fastify API uses **Open Router** as the default when `OPEN_ROUTER_API_KEY` is set (`meta-llama/llama-3.3-70b-instruct:free`). Use **Ollama** for data privacy—set `OLLAMA_BASE_URL` and optionally `AI_PROVIDER=ollama`.
