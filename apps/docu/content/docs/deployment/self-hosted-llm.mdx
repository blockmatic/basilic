...
By default, the app uses **Anthropic** (Claude Sonnet 4) when `ANTHROPIC_API_KEY` is set, otherwise **Open Router** (Claude Sonnet 4) or **Ollama**. Use `AI_DEFAULT_MODEL=openrouter/free` for OpenRouter's free tier. We recommend **self-hosting** with Ollama when data privacy is a priority—prompts and responses stay on your infrastructure, with no third-party API calls.
...
|----------|----------|---------|-------------|
| `ANTHROPIC_API_KEY` | Conditional | — | Anthropic API key; Claude Sonnet 4 (`claude-sonnet-4-20250514`) as default |
| `OPEN_ROUTER_API_KEY` | Conditional | — | Open Router API key (get at openrouter.ai); Claude Sonnet 4 as default |
