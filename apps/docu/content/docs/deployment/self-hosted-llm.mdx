...
The Fastify API uses **Open Router** as the default when `OPEN_ROUTER_API_KEY` is set (`meta-llama/llama-3.3-70b-instruct:free`). For privacy-first routing with Ollama, set `OLLAMA_BASE_URL` and, when both Open Router and Ollama are configured, you must set `AI_PROVIDER=ollama` because `getResolvedProvider()` prefers Open Router when `OPEN_ROUTER_API_KEY` exists.
