...
|----------|----------|---------|-------------|
| `OPEN_ROUTER_API_KEY` | No (one required) | — | Open Router API key (get at openrouter.ai) |
| `OLLAMA_BASE_URL` | No (for privacy) | `http://localhost:11434` | Ollama server URL when self-hosting |
| `AI_PROVIDER` | No | Inferred | `openrouter` or `ollama` — force when both configured |
