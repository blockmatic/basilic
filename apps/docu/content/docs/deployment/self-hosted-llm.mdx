...
By default, the app uses **Anthropic** (Claude 3.5 Sonnet) when `ANTHROPIC_API_KEY` is set, otherwise **Open Router** (`meta-llama/llama-3.3-70b-instruct:free`) or **Ollama**. We recommend **self-hosting** with Ollama when data privacy is a priority—prompts and responses stay on your infrastructure, with no third-party API calls.
...
The Fastify API supports **Anthropic**, **Open Router**, and **Ollama**. Provider precedence (when `AI_PROVIDER` is unset): Anthropic → Open Router → Ollama. Privacy-first routing with Ollama requires `AI_PROVIDER=ollama` when other keys exist. See `getResolvedProvider()` for the exact logic.
...
|----------|----------|---------|-------------|
| `ANTHROPIC_API_KEY` | Conditional | — | Anthropic API key; Claude 3.5 Sonnet (`claude-3-5-sonnet-20241022`) as default |
...
| `AI_DEFAULT_MODEL` | No | Varies by provider | Anthropic: `claude-3-5-sonnet-20241022`; Open Router: `meta-llama/llama-3.3-70b-instruct:free`; Ollama: `qwen3:8b` |
