...
Build → E2E. Triggers on pull request when `apps/web/**`, API routes the suite exercises (`auth`, `account`, `ai`, `health`, `test`), shared packages (including `packages/db/**`), OpenAPI, or the workflow/action files change. Single job: build `@repo/web...` and `@repo/db` (with localhost API URL), then E2E via `test:e2e:local` with `SKIP_BUILD=1` (spawns Fastify + Next with `ALLOW_TEST=true`, `RATE_LIMIT_MAX=10000`, `COINS_RATE_LIMIT_MAX=10000`, and `WEBAUTHN_RP_NAME=Test App`). Fastify imports Pattern B `@repo/db` `dist/`; `--filter=@repo/web...` does not build it. The chat Playwright project runs only when a non-placeholder Anthropic key is present (`hasRealAnthropicKey()`: empty, `sk-ant-xxx`, and `sk-ant-dummy*` omit it); forks without that secret still run auth and dashboard E2E. `cancel-in-progress: true`. Sets `TURBO_TOKEN` / `TURBO_TEAM` when configured.
...
Linting, `pnpm test:scripts`, and type checking on every PR (`pnpm lint`, then `pnpm test:scripts`, then `pnpm exec turbo run checktypes`). Supports `workflow_dispatch` for manual runs. Sets `TURBO_TOKEN` / `TURBO_TEAM` when configured.
