...
Build → E2E. Triggers on pull request when `apps/web/**`, `apps/api/src/routes/test/**`, shared packages, or openapi change. Single job: build (with localhost API URL), `test` (E2E-only placeholder, no unit suite), then E2E via `test:e2e:local` (spawns Fastify + Next with `ALLOW_TEST=true` and `RATE_LIMIT_MAX=10000`).
