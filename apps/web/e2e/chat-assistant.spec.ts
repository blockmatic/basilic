...
test.describe('Chat Assistant', () => {
  ...
  test('should render a market artifact from What moved?', async ({ page }) => {
    await page.goto('/')
    await expect(page.locator('text=Signed In')).toBeVisible({ timeout: 15_000 })
    await page.getByRole('button', { name: 'Open assistant' }).click()
    const sheet = page.getByRole('dialog')
    await expect(sheet).toBeVisible({ timeout: 5000 })
    await sheet.getByRole('button', { name: 'What moved?' }).click()

    const chatError = sheet.getByTestId('chat-error')
    const marketCard = sheet.getByTestId('market-card')
    const winner = await Promise.race([
      marketCard.waitFor({ state: 'visible', timeout: 60_000 }).then(() => 'card' as const),
      chatError.waitFor({ state: 'visible', timeout: 60_000 }).then(() => 'error' as const),
    ])
    if (winner === 'error') {
      ...
      if (isCi) throw new Error(`Chat failed in CI: ${errorText || '(no error text)'}`)
      throw new Error(`Chat failed: ${errorText || '(no error text)'}`)
    }
