...
Copy [`.env.defaults.example`](.env.defaults.example) to `.env` and set values (gitignored). Optional `COINGECKO_DEMO_API_KEY` and `MARKETS_CACHE_MS` configure `@repo/markets` (CoinGecko Demo + Binance public REST). JWT `GET /coins` and `POST /coins/query` filter cached `getMarkets` (fixture on vendor 429). JWT watch CRUD is `GET/PUT/DELETE /coins/watches` by asset id. JWT `GET /agents` lists eve command/chat origins. Start database first (`pnpm db:start`), then **`pnpm reset`** from the repo root (or `pnpm reset` here) before `pnpm dev`. Uses Supabase CLI for PostgreSQL, or `PGLITE=true` for in-memory. Dev server at [http://localhost:3001](http://localhost:3001). Adopter bar: [Product Ready](../docu/content/docs/testing/product-ready.mdx).
