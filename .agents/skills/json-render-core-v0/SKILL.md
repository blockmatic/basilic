---
name: json-render-core-v0
description: Core package for defining schemas, catalogs, and specs for json-render. Use when working with @json-render/core, defining catalogs, composing or validating Spec trees, or using $state bindings.
---
...
- Applies to: `@json-render/core` 0.x (`defineSchema`, `defineCatalog`, spec validation, `$state`)
- Does NOT cover: React rendering (see [json-render-react-v0](../json-render-react-v0/SKILL.md)), `@json-render/next` app shells, `@json-render/shadcn`, MCP Apps

## Assumptions

- Folder major tracks the installed `@json-render/core` major (0.x)
- Specs are validated against the catalog before paint
- Application TypeScript owns composition when the product maps a closed config to a Spec

## Principles

- Catalog + registry are the guardrails; prices and other live data live in `$state`, not in the spec
- Prefer a deterministic compose function over LLM `generateObject` of a spec tree
- `experimental_composeSpec` / `experimental_createEvaluator` stay optional; read installed package docs before calling them

## Constraints

### MUST

- Validate every spec against the catalog before render
- Bind dynamic values with `$state` / `repeat` rather than embedding live rows in the spec

### AVOID

- `@json-render/next` `createNextApp` as the product shell
- `@json-render/mcp` as a public product surface
- Treating `experimental_composeSpec` as required for a first catalog

## Interactions

- Complements [json-render-react-v0](../json-render-react-v0/SKILL.md), [typesafe-ai](../typesafe-ai/SKILL.md), [shadcn-v3](../shadcn-v3/SKILL.md)
