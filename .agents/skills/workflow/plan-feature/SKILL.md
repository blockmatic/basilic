...
9. **Persist**: Keep the Cursor plan file as the working brief. If the change names product intent, also patch the Product instance in `_first/FIRST.md` (Basilic: `_first/basilic/PRODUCT.md`) in the same implementation PR. Do not put the plan only in `__dev/` or chat. Backlog items go to GitHub Issues, not `BACKLOG.md`.
