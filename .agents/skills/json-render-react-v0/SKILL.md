---
name: json-render-react-v0
description: React renderer that turns json-render Spec trees into component trees. Use when working with @json-render/react, defineRegistry, Renderer, or StateProvider.
---
...
- Applies to: `@json-render/react` 0.x (`defineRegistry`, `Renderer`, state providers)
- Does NOT cover: catalog schema authoring (see [json-render-core-v0](../json-render-core-v0/SKILL.md)), Next app generators, shadcn wholesale packs

## Assumptions

- Folder major tracks the installed `@json-render/react` major (0.x)
- Registry components are trusted application UI, not generated JS

## Principles

- Map catalog names to existing design-system components
- Keep the renderer inside an existing app island; do not replace the App Router shell

## Constraints

### MUST

- Register only actions the host implements
- Pass `$state` from the host (API/cache), not from model output

### AVOID

- `@json-render/next` as the app
- Spreading `@json-render/shadcn` wholesale when the host already has a component library

## Interactions

- Complements [json-render-core-v0](../json-render-core-v0/SKILL.md), [shadcn-v3](../shadcn-v3/SKILL.md), [next-v16](../next-v16/SKILL.md), [nuqs-v2](../nuqs-v2/SKILL.md)
