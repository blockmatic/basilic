...
Token groups may follow W3C Design Tokens (DTCG) conceptually. Prefer the project's pinned lint command when the CLI is available. Use `npx @google/design.md lint _first/DESIGN.md` (explicit path) only when the CLI is not already part of the toolchain. A zero exit code is not an accessibility pass or browser verification.
