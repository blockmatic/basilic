...
Generated projects retain committed stack skills and `skills-lock.json` pinned to `mattpocock/skills` (recommended daily workflow) and `blockmatic/basilic-skills` (`w-*` alternative); `pnpm setup` installs those catalogs into `.agents/skills/` and restores the lock. Documentation pointers are rewritten to the local snapshot. The agent contract (`AGENTS.md`, `.agents/rules/always.md`) ships with the tree.
